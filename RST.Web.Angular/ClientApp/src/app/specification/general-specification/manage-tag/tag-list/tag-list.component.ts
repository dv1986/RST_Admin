...
export class TagListComponent implements OnInit {
    ...
    onUpdate() {
        ...
        this._specificationService.updateProductTag(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._specificationService.deleteProductTag(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(TagAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Tag Name', field: 'name', sortable: true, filter: true, type: ['editableColumn'] },
