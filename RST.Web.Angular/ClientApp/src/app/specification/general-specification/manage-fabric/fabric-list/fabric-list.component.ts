...
export class ProductFabricListComponent implements OnInit {
    ...
    onUpdate() {
        ...
        this._specificationService.updateProductFabric(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._specificationService.deleteProductFabric(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(FabricAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Fabric Name', field: 'fabricName', sortable: true, filter: true, type: ['editableColumn'] },
