...
import { MeasureDimensionAddComponent } from '../measure-dimension-add/measure-dimension-add.component';
...
@Component({
    selector: 'app-measure-dimension-list',
    templateUrl: './measure-dimension-list.component.html'
})
export class MeasureDimensionListComponent implements OnInit {
    ...
    onUpdate() {
        ...
        this._specificationService.updateMeasureDimension(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._specificationService.deleteMeasureDimension(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(MeasureDimensionAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'System Keyword', field: 'systemKeyword', sortable: true, filter: true, type: ['editableColumn'] },
            {
                headerName: 'Ratio', field: 'ratio', type: ['editableColumn','Decimal'],
                cellEditor: "numericCellEditor"
            },
            { headerName: 'Display Order', field: 'displayOrder', sortable: true, type: ['editableColumn', 'numberColumn'], cellEditor: "numericCellEditor" },
