...
export class BrandListComponent implements OnInit {
    ...
    onUpdate() {
        ...
        this._productService.updateBrands(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._productService.deleteBrands(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(BrandAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Short Name', field: 'shortName', sortable: true, filter: true, type: ['editableColumn'] },
            { headerName: 'Full Name', field: 'fullName', sortable: true, filter: true, type: ['editableColumn'] },
            { headerName: 'Description', field: 'description', sortable: true, filter: true, type: ['editableColumn'] },
