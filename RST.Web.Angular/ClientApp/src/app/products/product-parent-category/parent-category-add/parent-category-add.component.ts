...
@Component({
    selector: 'parent-category-add.component',
    templateUrl: './parent-category-add.component.html'
})
export class ParentCategoryAddComponent implements OnInit {
    ...
    initForm() {
        this.form = new FormGroup({
            'productCategoryParentId': new FormControl(''),
            'catParentName': new FormControl('', Validators.required),
    ...
    onSubmit() {
        ...
        this.model.catParentName = this.form.controls['catParentName'].value;
        ...
        this._productService.addProductCategoryParent(this.model).subscribe(result => {
            if (result.state == 0) {
