...
import { ParentCategoryAddComponent } from '../parent-category-add/parent-category-add.component';


@Component({
    selector: 'parent-category-list',
...
export class ParentCategoryListComponent implements OnInit {
    ...
    onUpdate() {
        ...
        this._productService.updateProductCategoryParent(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._productService.deleteProductCategoryParent(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(ParentCategoryAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-lg" })
        if (this.modalRef) {
    ...
    onImageClick(params) {
        ...
        const initialState = {
            ...
            //ImagePath: params.data.imageFileStream,
            ImageName: params.data.imageName,
            moduleName: "ParentCategory",
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Cat Parent Name', field: 'catParentName', sortable: true, filter: true, type: ['editableColumn'] },
