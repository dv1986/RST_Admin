...
export class SubCategoryAddComponent implements OnInit {
    ...
    onCategoryChange(params) {
        this._productService.getSubCategoryParentLookup(params.rowId).subscribe(response => {
            if (response.state == 0) {
                this.parentSubCategories = response.data;
            }
        });
    }
    ...
    onSubmit() {
        ...
        this.model.subCategoryName = this.form.controls['subCategoryName'].value;
        ...
        this._productService.addProductSubCategory(this.model).subscribe(result => {
            if (result.state == 0) {
