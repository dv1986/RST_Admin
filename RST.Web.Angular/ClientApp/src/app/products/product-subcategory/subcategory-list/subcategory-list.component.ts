...
export class SubCategoryListComponent implements OnInit {
    ...
    loadData() {
        this._productService.getProductSubCategory("").subscribe(response => {
            if (response.state == 0)
    ...
    onUpdate() {
        ...
        this._productService.updateProductSubCategory(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._productService.deleteProductSubCategory(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(SubCategoryAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-lg" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'CategoryParentId', field: 'categoryParentId', hide: true },
            { headerName: 'CategoryId', field: 'categoryId', hide: true },
            { headerName: 'ProductSubCategoryParentId', field: 'productSubCategoryParentId', hide: true },
            { headerName: 'Cat Parent Name', field: 'categoryParentName' },
