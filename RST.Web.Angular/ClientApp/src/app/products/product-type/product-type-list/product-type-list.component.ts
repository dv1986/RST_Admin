...
export class ProductTypeListComponent implements OnInit {
    ...
    loadData() {
        this._productService.getProductType("").subscribe(response => {
            if (response.state == 0)
    ...
    onUpdate() {
        ...
        this._productService.updateProductType(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._productService.deleteProductType(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(ProductTypeAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-lg" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'ProductSubCategoryParentId', field: 'productSubCategoryParentId', hide: true },
            { headerName: 'ProductSubCategoryId', field: 'productSubCategoryId', hide: true },
            { headerName: 'Category Name', field: 'categoryName' },
            { headerName: 'Sub Category Parent Name', field: 'subCategoryParentName' },
            { headerName: 'Sub Category Name', field: 'subCategoryName' },
            { headerName: 'Product Type', field: 'name', type: ['editableColumn'],sortable: true, filter: true },
            // {
            //     headerName: 'Sub Category Parent', field: 'subCategoryParent', sortable: true, filter: true, type: ['editableColumn'], cellEditor: 'agSelectCellEditor',
            ...
            // },
            // { headerName: 'Sub-Category Name', field: 'subCategoryName', sortable: true, filter: true, type: ['editableColumn'] },
            { headerName: 'Size Type', field: 'typeName', sortable: true, filter: true },
        ...
        this._productService.getProductSubCategoryParent("").subscribe(response => {
            if (response.state == 0) {
                response.data.forEach(element => {
                    this.CategoryList.push(`${element.rowId}-${element.subCategoryParentName}`);
                });
            }
        });
    }
    ...
    lookUpValueGetterforCategory(params) {
        if (params.data !== undefined) {
            return params.data.subCategoryParent;
        }
    }
    ...
    lookUpValueSetterforCategory(params) {
        if (params.data !== undefined) {
            ...
            params.data.productSubCategoryParentId = Number(id);

            var text = params.newValue.split('-')[1].trim();
            params.data.subCategoryParent = text;
            return params.data.subCategoryParent;
        }
    }
}
