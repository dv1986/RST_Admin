...
import { ProductAttributeParentAddComponent } from '../product-attribute-parent-add/product-attribute-parent-add.component';
import { AttributeService } from '../../attributes.service';


@Component({
    selector: 'app-product-attribute-parent-list',
    templateUrl: './product-attribute-parent-list.component.html'
})
export class ProductAttributeParentListComponent implements OnInit {
    ...
    onUpdate() {
        ...
        this._attributeService.updateproductAttributeParent(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._attributeService.deleteproductAttributeParent(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(ProductAttributeParentAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Attribute Parent Name', field: 'name', sortable: true, filter: true, type: ['editableColumn'] },
