...
export class MenuListComponent implements OnInit {
    ...
    loadData() {
        this._masterService.getMenu("").subscribe(response => {
            if (response.state == 0)
    ...
    onUpdate() {
        ...
        this._masterService.updateMenu(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._masterService.deleteMenu(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(MenuAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Menu Name', field: 'menuName', sortable: true, filter: true, type: ['editableColumn'] },
            { headerName: 'Icon Name', field: 'iconName', sortable: true, filter: true, type: ['editableColumn'] },
