...
export class CityListComponent implements OnInit {
    ...
    filterComponent: GridFilterItem = new GridFilterItem(CitySearchComponent, this);
    ...
    loadData() {
        if (this.searchCriteria.countryId === null)
            this.searchCriteria.countryId = 0;
        if (this.searchCriteria.stateId === null)
            this.searchCriteria.stateId = 0;
        this._masterService.getCity(this.searchCriteria.countryId, this.searchCriteria.stateId).subscribe(response => {
            if (response.state == 0)
    ...
    onUpdate() {
        ...
        this._masterService.updateCity(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();this.notificationService.ShowSuccess("Record updated sucessfully!", 3000);
            }
    ...
    onDelete() {
        ...
        this._masterService.deleteCity(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    resetLocalStorage() {
        localStorage.removeItem("citySearchQuery");
        this.searchCriteria.countryId = 0;
        this.searchCriteria.stateId = 0;
    }
    ...
    loadSearchCritariaFromLocalStorage() {
        if (localStorage.getItem('citySearchQuery')) {
            ...
            this.searchCriteria.countryId = searchRequest.countryId;
            this.searchCriteria.stateId = searchRequest.stateId;
        }
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'State Name', field: 'stateName', sortable: true, filter: true },
            { headerName: 'City Name', field: 'cityName', sortable: true, filter: true, type: ['editableColumn'] },
