...
import { SeoContentAddComponent } from '../seo-content-add/seo-content-add.component';


@Component({
    selector: 'app-seo-content-list',
    templateUrl: './seo-content-list.component.html'
})
export class SeoContentListComponent implements OnInit {
    ...
    loadData() {
        this._masterService.getSeoContent("").subscribe(response => {
            if (response.state == 0)
    ...
    onUpdate() {
        ...
        this._masterService.updateSeoContent(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._masterService.deleteSeoContent(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(SeoContentAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Meta Title', field: 'metaTitle', sortable: true, filter: true, type: ['editableColumn'] },
            { headerName: 'Meta Keyword', field: 'metaKeyword', sortable: true, filter: true, type: ['editableColumn'] },
            { headerName: 'Meta Description', field: 'metaDescription', sortable: true, filter: true, type: ['editableColumn'] },
