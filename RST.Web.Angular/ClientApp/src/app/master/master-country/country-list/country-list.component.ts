...
export class CountryListComponent implements OnInit {
    ...
    loadData() {
        this._masterService.getCountry(this.searchCriteria.countryName).subscribe(response => {
            if (response.state == 0)
    ...
    onUpdate() {
        ...
        this._masterService.updateCountry(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._masterService.deleteCountry(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew() {
        ...
        this.modalRef = this.modalService.show(CountryAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    resetLocalStorage() {
        localStorage.removeItem("countrySearchQuery");
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Country Name', field: 'countryName', sortable: true, filter: true, type: ['editableColumn'] },
