...
import { SubMenuAddComponent } from '../submenu-add/submenu-add.component';
...
export class SubMenuListComponent implements OnInit {
    ...
    loadData() {
        this._masterService.getSubMenu("").subscribe(response => {
            if (response.state == 0)
    ...
    onUpdate() {
        ...
        this._masterService.updateSubMenu(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._masterService.deleteSubMenu(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onAddNew(){
        ...
        this.modalRef = this.modalService.show(SubMenuAddComponent, { initialState: initialState, backdrop: 'static', keyboard: false, animated: true, ignoreBackdropClick: true, class: "modal-md" })
        if (this.modalRef) {
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Menu Name', field: 'menuName', sortable: true, filter: true },
            { headerName: 'Sub-Menu Name', field: 'subMenuName', sortable: true, filter: true, type: ['editableColumn'] },
