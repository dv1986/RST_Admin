...
export class FormListComponent implements OnInit {
    ...
    loadData() {
        this._formBuilderService.getFormBuilder(0).subscribe(response => {
            if (response.state == 0)
    ...
    onUpdate() {
        ...
        this._formBuilderService.updateFormBuilder(this.gridInformation.updatedData).subscribe(response => {
            if (response.state == 0) {
                this.onRefresh();
    ...
    onDelete() {
        ...
        this._formBuilderService.deleteFormBuilder(this.gridInformation.selectedData).subscribe(response => {
            if (response.state == 0) {
    ...
    onSelectionChanged(params) {
        localStorage.setItem("FormBuilderData", JSON.stringify(this.gridInformation.selectedData[0]));
    }
    ...
    initGrid() {
        this.gridOptions.columnDefs = [
            ...
            { headerName: 'Form Name', field: 'formName', editable: true, type: ['editableColumn'] },
            { headerName: 'Form Json', field: 'formJson' },
