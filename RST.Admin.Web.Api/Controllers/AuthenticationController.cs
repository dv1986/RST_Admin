    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public IActionResult Authenticate(string userRef, string password)
        {
            ...
            catch (Exception exception)
            {
                ...
                _logger.LogError(exception, "Error Getting Authenticate==>" + exception.StackTrace, userRef);
            }
        ...
        [HttpPost]
        [Route("VerifyOTP")]
        public IActionResult VerifyOTP(string SessionId, string OTP)
        {
            var response = new OperationResponse<string>();
            try
            {
            ...
            catch (Exception exception)
            {
                response.State = ResponseState.Error;
                response.Messages.Add(exception.Message + " StackTrace==> " + exception.StackTrace);
                _logger.LogError(exception, "Error Getting GenerateOTP==>" + exception.StackTrace, SessionId, OTP);
            }
            return new JsonResult(response);
        }
    }
